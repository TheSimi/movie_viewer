...
class Movie(Media):
    ...
    def play(self, media_player: str = MEDIA_PLAYER, speed: float = 1):
        ...
        if self.is_file:
            ...
            if self.is_vlc(media_player):
                ...
                logger.warning(
                    f"Opening movie folder {self.path} because the media player {media_player} is not VLC and not supported for playing folders"
                )
    ...
    def _play_file_with_subtitles_vlc(
        ...
        logger.info(
            f"Playing movie file {video_file} with subtitle file {sub_file} using {media_player} at speed {speed}"
        )
