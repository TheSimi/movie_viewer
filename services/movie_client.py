...
class MovieClient(ApiClient):
    ...
    def search_media(cls, title: str):
        try:
            ...
            logger.warning(f"Failed to search for movie using fm-db api with title {title}: {e.__class__.__name__} | {e}")
    ...
    def get_media(cls, id: str, title: Optional[str] = None, **kwargs):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to fetch data with imdb.dev api for {id}: {e.__class__.__name__} | {e}")
                if title:
    ...
    def get_media_by_title(cls, title: str):
        ...
        try:
            ...
            logger.warning(f"Failed to fetch data for title {title}: {e.__class__.__name__} | {e}")
            return {}
    ...
    def get_poster(cls, id: str, title: Optional[str] = None, **kwargs):
        ...
        try:
            ...
            logger.warning(f"Failed to fetch poster with fm-db api for {id}: {e.__class__.__name__} | {e}")
            if title:
                try:
                    return cls.get_poster_by_title(title)
                except Exception as e:
                    logger.warning(f"Failed to fetch poster by title {title}: {e.__class__.__name__} | {e}")
            try:
                return ImdbdevClient.get_poster(id)
            except Exception as e:
                logger.warning(f"Failed to fetch poster with imdb.dev api for {id}: {e.__class__.__name__} | {e}")
